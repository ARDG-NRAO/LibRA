...
class Hpg(CMakePackage):
    ...
    version("1.2.3", commit="2afac2a1a3d02c9c90d3bd64afa6ecf5b7182102")
    version("1.2.2", commit="3291b9a3218fe5876cafbefd730d34e886b2426a")

    variant(
        "api",
        default="17",
        description="C++ standard for API",
        values=("11", "17"),
        multi=False,
    )
    variant("cuda", default=False, description="Enable CUDA device")
    variant(
        "exptl", default=False, description="Enable experimental kernel implementations"
    )
    variant("max_cf_grps", default="1000", description="Maximum number of CF groups")
    variant(
        "mixed", default=False, description="Use mixed precision in gridding kernels"
    )
    ...
    variant(
        "array_index",
        default="uint",
        description="Array index value type",
    ...
    variant(
        "cf_fp",
        default="float",
        description="CF value base fp type",
        values=("float", "double"),
        multi=False,
    )
    variant(
    ...
    variant(
        "flgbits",
        default="8",
        description="Flag value number of bits",
        values=("8", "16"),
        multi=False,
    )
    ...
    variant(
        "uvw_rank",
        default="2",
        description="UVW values array rank",
        values=("2", "3"),
        multi=False,
    )
    variant(
        "cfidx_rank",
        default="2",
        description="CF arrays index value rank",
        values=("1", "2"),
        multi=False,
    )

    conflicts("~openmp", when="~serial", msg="At least one host device must be enabled")
    conflicts(
        "~exptl",
        when="+mixed",
    ...
    def common_cmake_args(self):
        return [
            ...
            self.define_from_variant(
                "Hpg_ENABLE_EXPERIMENTAL_IMPLEMENTATIONS", "exptl"
            ),
            self.define(
