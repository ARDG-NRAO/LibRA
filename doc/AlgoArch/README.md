...
The relationship between the raw data and the image of the sky is expressed via a linear equation $V = G A I + n$.  $V$ is the raw data (the visibilities), $G$ represents the multiplicative corrections (instrumental or atmospheric), $I$ is a representations of the sky brightness distribution and $n$ is the additive noise with Normal probably distribution.  $A$ is an operator that transforms $I$ to the data domain.  The goal of calibration algorithms is to correct for the effects of $G$, and the goal of imaging algorithms is derive $I$ given $V$, $G$ and a statistical description of $n$.
